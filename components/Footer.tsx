    ...
    <footer className="mt-20 bg-white dark:bg-slate-900">
      <div className="mx-auto max-w-screen-xl px-6 pt-16 sm:px-8">
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-6 md:gap-8">
          <div className="flex flex-col md:col-span-2">
            <span className="text-xl font-bold capitalize">ZhukOFF</span>
            <p className="mt-4 max-w-screen-sm">
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptate
              rem eligendi illum repudiandae vel tenetur? Facilis sed, amet
              placeat explicabo deleniti rem nesciunt, pariatur distinctio non
              quas velit, iusto vero!
            </p>
            <ul className="mt-4 flex items-center space-x-4" id="social-links">
              {SocialLinks.map((item) => (
                <li key={item.title}>
                  <a
                    href={item.link}
                    aria-label={item.title}
                    className="transition-all duration-150 hover:fill-indigo-800 
                    dark:fill-white dark:hover:fill-indigo-800"
                  >
                    {item.Icon}
                  </a>
                </li>
            ...
            <span className="mt-8 text-xl font-semibold capitalize">
                ...
                <ul className="mt-4 space-y-2">
                  {items.map((item, i) => (
                    <li key={i}>
                      <Link href={item.href}>
                        <a
                          className="transition-all duration-150 hover:text-indigo-800
                        hover:underline dark:text-gray-300 dark:hover:text-indigo-400"
                        >
                          {item.title}
                        </a>
                      </Link>
      ...
      >
        <p className="text-center text-sm dark:text-gray-300">
          Copyright © 2022{' '}
          <a
            href="https://github.com/Zhuk-off/NextJs_TailWind_Frontend"
